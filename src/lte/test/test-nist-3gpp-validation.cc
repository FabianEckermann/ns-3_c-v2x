...
class SaveTraces : public Object
{
public:
    SaveTraces (std::string path);
    ~SaveTraces ();
...
SaveTraces::SaveTraces (std::string path)
{
...
void 
SaveTraces::GetGainTrace  (std::string context, Ptr<MobilityModel> tx,Ptr<MobilityModel> rx, double txAntennaGain, double rxAntennaGain, double propagationGain, double pathloss)
{
...
main (int argc, char *argv[])
{
  ...
  Config::SetDefault ("ns3::LteHelper::HandoverAlgorithm", StringValue ("ns3::A3RsrpHandoverAlgorithm"));
  ...
  */
  ...
  // Set pathloss model
  lteHelper->SetAttribute ("PathlossModel", StringValue ("ns3::NistUrbanmacrocellPropagationLossModel"));
  ...
  SaveTraces dlFileTrace ("dl-gain.out");
  SaveTraces ulFileTrace ("ul-gain.out");
  Config::Connect ("/ChannelList/0/Gain", MakeCallback (&SaveTraces::GetGainTrace, &dlFileTrace));
  Config::Connect ("/ChannelList/1/Gain", MakeCallback (&SaveTraces::GetGainTrace, &ulFileTrace));

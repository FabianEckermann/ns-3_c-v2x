...
NS_LOG_COMPONENT_DEFINE ("LteProseHelper");

NS_OBJECT_ENSURE_REGISTERED (LteProseHelper);

LteProseHelper::LteProseHelper (void)
{
...
TypeId LteProseHelper::GetTypeId (void)
{
  static TypeId
    tid = 
    TypeId ("ns3::LteProseHelper")
    .SetParent<Object> ()
    .SetGroupName("Lte")
    .AddConstructor<LteProseHelper> ()
  ;
  return tid;
}

void
LteProseHelper::DoDispose ()
{
  NS_LOG_FUNCTION (this);
  Object::DoDispose ();
}
...
void 
LteProseHelper::SetLteHelper (Ptr<LteHelper> h)
{
...
NetDeviceContainer 
LteProseHelper::RemoveNetDevice (NetDeviceContainer container, Ptr<NetDevice> item)
{
...
LteProseHelper::AssociateForGroupcast (double txPower, double ulEarfcn, double ulBandwidth, NetDeviceContainer ues, double rsrpThreshold, int ngroups, int nreceivers, SrsrpMethod_t compMethod)
{
      ...
      while (numReceiversAssociated < nreceivers && candidateRx.GetN() > 0)
        {
          ...
          double rsrpRx = 0;
          if (compMethod == LteProseHelper::SRSRP_STD)
            {
...
std::vector < NetDeviceContainer > 
LteProseHelper::AssociateForBroadcast (double txPower, double ulEarfcn, double ulBandwidth, NetDeviceContainer ues, double rsrpThreshold, uint32_t ntransmitters, SrsrpMethod_t compMethod)
{
  std::vector < NetDeviceContainer > groups; //groups created
  Ptr<UniformRandomVariable> ueSelector = CreateObject<UniformRandomVariable> (); //random variable to select Tx/Rx nodes
...
std::vector<NetDeviceContainer>
LteProseHelper::AssociateForVehicularBroadcast(double txPower, double ulEarfcn, double ulBandwidth, NetDeviceContainer ues, double rsrpThreshold, uint32_t ntransmitters, SrsrpMethod_t compMethod)
{
	std::vector < NetDeviceContainer > groups; //groups created
	NetDeviceContainer remainingUes; //list of UEs not assigned to groups
	remainingUes.Add (ues);
...
std::vector<NetDeviceContainer>
LteProseHelper::AssociateForPlatoon(double txPower, double ulEarfcn, double ulBandwidth, NetDeviceContainer ues, double rsrpThreshold, uint32_t ntransmitters, SrsrpMethod_t compMethod)
{
	std::vector < NetDeviceContainer > groups; //groups created
	NetDeviceContainer remainingUes; //list of UEs not assigned to groups
	remainingUes.Add (ues);
...
std::vector < NetDeviceContainer > 
LteProseHelper::AssociateForBroadcastWithTxEnabledToReceive (double txPower, double ulEarfcn, double ulBandwidth, NetDeviceContainer ues, double rsrpThreshold, uint32_t ntransmitters, SrsrpMethod_t compMethod)
{
  std::vector < NetDeviceContainer > groups; //groups created
  Ptr<UniformRandomVariable> ueSelector = CreateObject<UniformRandomVariable> (); //random variable to select Tx/Rx nodes
...
std::vector < NetDeviceContainer > 
LteProseHelper::AssociateForBroadcastWithWrapAround (double txPower, double ulEarfcn, double ulBandwidth, NetDeviceContainer ues, double rsrpThreshold, uint32_t ntransmitters, Ptr<LteHexGridEnbTopologyHelper> topologyHelper, SrsrpMethod_t compMethod)
{
  std::vector < NetDeviceContainer > groups; //groups created
  Ptr<UniformRandomVariable> ueSelector = CreateObject<UniformRandomVariable> (); //random variable to select Tx/Rx nodes
...
void
LteProseHelper::PrintGroups (std::vector < NetDeviceContainer > groups)
{
...
void
LteProseHelper::PrintGroups (std::vector < NetDeviceContainer > groups, Ptr< OutputStreamWrapper > stream)
{
...
void 
LteProseHelper::ActivateSidelinkBearer (Time activationTime, NetDeviceContainer ues, Ptr<LteSlTft> tft)
{
  NS_LOG_FUNCTION (this);
  NS_ASSERT_MSG (m_lteHelper, "Sidelink activation requires LteHelper to be registered with the LteProseHelper");
  Simulator::Schedule (activationTime, &LteProseHelper::DoActivateSidelinkBearer, this, ues, tft);
}

void 
LteProseHelper::DoActivateSidelinkBearer (NetDeviceContainer ues, Ptr<LteSlTft> tft)
{
  NS_LOG_FUNCTION (this);

...
NS_LOG_COMPONENT_DEFINE ("Lte3gppPropagationLossModel");

namespace ns3 {

NS_OBJECT_ENSURE_REGISTERED (Lte3gppPropagationLossModel);


Lte3gppPropagationLossModel::Lte3gppPropagationLossModel ()
{
  ...
  m_urbanmicrocell = CreateObject<CniUrbanmicrocellPropagationLossModel> (); 
}
...
TypeId
Lte3gppPropagationLossModel::GetTypeId (void)
{
  static TypeId
    tid =
    TypeId ("ns3::Lte3gppPropagationLossModel")
    .SetParent<BuildingsPropagationLossModel> ()
    .AddConstructor<Lte3gppPropagationLossModel> ()
    .AddAttribute ("Frequency",
                   "The propagation frequency  (default : general cases 2100MHz band).",
                   DoubleValue (2106e6),
                   MakeDoubleAccessor (&Lte3gppPropagationLossModel::SetFrequency),
                   MakeDoubleChecker<double> ())
    /*.AddAttribute ("LosCheck",
                   "LOS/NLOS condition",
                   BooleanValue (false),
                   MakeBooleanAccessor (&Lte3gppPropagationLossModel::m_los),
                   MakeBooleanChecker ())*/
    .AddAttribute ("CacheLoss",
                   "Indicates if the loss value should be cached",
                   BooleanValue (false),
                   MakeBooleanAccessor (&Lte3gppPropagationLossModel::m_cacheLoss),
                   MakeBooleanChecker ())    
    .AddTraceSource ("PathlossValue",
                     "Pathloss value to trace",
                     MakeTraceSourceAccessor (&Lte3gppPropagationLossModel::m_pathlossTrace),
                     "ns3::Lte3gppPropagationLossModel::PathlossValueTracedCallback")
  ;
  return tid;
}

void
Lte3gppPropagationLossModel::SetFrequency (double freq)
{
...
double
Lte3gppPropagationLossModel::GetLoss (Ptr<MobilityModel> a, Ptr<MobilityModel> b) const
{
  ...
  else
    {
      ...
      if (it_b != m_lossMap.end ())
      ...
      else
        {
          ...
          // LTE
          if (((a->GetObject<Node> ()->GetDevice (0)->GetObject<LteUeNetDevice> () != 0) and (b->GetObject<Node> ()->GetDevice (0)->GetObject<LteEnbNetDevice> () != 0)) or ((b->GetObject<Node> ()->GetDevice (0)->GetObject<LteUeNetDevice> () != 0) and (a->GetObject<Node> ()->GetDevice (0)->GetObject<LteEnbNetDevice> () != 0)))
            {
              loss = Urbanmacrocell (a,b);
            }
          //D2D or V2X
          else if ((a->GetObject<Node> ()->GetDevice (0)->GetObject<LteUeNetDevice> () != 0) and (b->GetObject<Node> ()->GetDevice (0)->GetObject<LteUeNetDevice> () != 0))
            {
...
double
Lte3gppPropagationLossModel::GetShadowing (Ptr<MobilityModel> a, Ptr<MobilityModel> b)
const
{
    Ptr<MobilityBuildingInfo> a1 = a->GetObject <MobilityBuildingInfo> ();
    Ptr<MobilityBuildingInfo> b1 = b->GetObject <MobilityBuildingInfo> ();
...
double
Lte3gppPropagationLossModel::EvaluateSigma (Ptr<MobilityBuildingInfo> a, Ptr<MobilityBuildingInfo> b) const
{
  //LTE
  if (((a->GetObject<Node> ()->GetDevice (0)->GetObject<LteUeNetDevice> () != 0) and (b->GetObject<Node> ()->GetDevice (0)->GetObject<LteEnbNetDevice> () != 0)) or ((b->GetObject<Node> ()->GetDevice (0)->GetObject<LteUeNetDevice> () != 0) and (a->GetObject<Node> ()->GetDevice (0)->GetObject<LteEnbNetDevice> () != 0)))
  {
...
Lte3gppPropagationLossModel::Indoor (Ptr<MobilityModel> a, Ptr<MobilityModel> b) const
{
  return m_indoor->GetLoss (a, b);
}

double
Lte3gppPropagationLossModel::Outdoor (Ptr<MobilityModel> a, Ptr<MobilityModel> b) const
{
  return m_outdoor->GetLoss (a, b);
}

double
Lte3gppPropagationLossModel::Hybrid (Ptr<MobilityModel> a, Ptr<MobilityModel> b) const
{
  return m_hybrid->GetLoss (a, b);
}

double
Lte3gppPropagationLossModel::Urbanmacrocell (Ptr<MobilityModel> a, Ptr<MobilityModel> b) const
{
  return m_urbanmacrocell->GetLoss (a, b);
}

double 
Lte3gppPropagationLossModel::Urbanmicrocell (Ptr<MobilityModel> a, Ptr<MobilityModel> b) const
{
